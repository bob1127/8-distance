...
export default function ServiceClient() {
  const serviceRef = useRef(null); // 服務項目（流程）
  const chargeRef = useRef(null); // 收費標準

  const [activeTab, setActiveTab] = useState("service"); // "service" | "charge"
  const [showSwitch, setShowSwitch] = useState(true); // 控制整個 switch 是否顯示

  // 偵測滾動方向：往下 → 隱藏；往上 → 顯示（整個消失再出現）
  useEffect(() => {
  ...
  const scrollToRef = (ref) => {
    ...
    const targetY =
      ref.current.getBoundingClientRect().top + window.scrollY - 80; // 視需要調整偏移
    ...
    <div className="relative min-h-[200vh]">
      ...
      <section className="section-hero pt-20">
        <div className="relative py-10">
          {/* 底部固定 switch（整個消失／再出現） */}
          <AnimatePresence>
            {showSwitch && (
              <motion.div
                key="switch"
                className="fixed z-[9999999] left-0 bottom-6 w-full"
              ...
              >
                <div className="relative w-full">
                  <div className="switch-bar max-w-[260px] bg-[#f5f5f7] mx-auto px-2 py-2 flex rounded-[30px] shadow-sm border border-black/5">
      ...
      <section ref={serviceRef} className="section-process my-20">
        ...
        <div className="grid max-w-[1200px] mx-auto grid-cols-2">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="item justify-center items-center flex my-5">
