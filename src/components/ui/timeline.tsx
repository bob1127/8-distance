    ...
    >
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          <div
            key={index}
            className="flex flex-col md:flex-row items-start gap-6 md:gap-10 pt-16 relative"
          >
            {/* 時間軸原點與標題區塊 */}
            <div className="flex items-center md:items-start md:flex-row flex-col gap-2 md:gap-0 md:sticky md:top-40 w-full md:w-auto z-30">
              {/* 原點：在手機置中 */}
              <div className="relative md:static w-full md:w-auto">
                <div className="absolute md:relative left-1/2 -translate-x-1/2 md:left-3 md:translate-x-0 h-10 w-10 flex items-center justify-center">
                  <div className="h-4 w-4 rounded-full bg-green-600 shadow" />
                </div>
              </div>
              {/* 標題文字 */}
              <h3 className="text-center md:text-left text-base md:text-xl font-bold text-neutral-600 dark:text-neutral-400 md:pl-6 pt-12 md:pt-0">
                {item.title}
              </h3>
            </div>
            ...
            <div className="w-full px-4 md:px-0">
              <h3 className="md:hidden text-base font-bold text-neutral-500 mb-2">
                {item.title}
              </h3>
              {item.content}
            </div>
